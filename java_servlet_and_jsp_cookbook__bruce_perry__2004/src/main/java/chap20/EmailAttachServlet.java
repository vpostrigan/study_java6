...
public class EmailAttachServlet extends HttpServlet {
    ...
    //default value for mail server address, in case the user doesn't provide one
    ...
    private void sendMessage(String smtpServ, String to, String from,
        ...
        //populate the 'Properties' object with the mail server address, so that the default 'Session' instance can use it.
        ...
        try {
            if (to != null) {
                //throws 'AddressException' if the 'to' email address violates RFC822 syntax
            ...
            FileDataSource fds = new FileDataSource(new File("h:/book/chapters/chap1/chap1.doc"));
            ...
            //the syntax here is the MIME type followed by space separated extensions
            ftm.addMimeTypes("application/msword doc DOC");

            fds.setFileTypeMap(ftm);
            //The DataHandler is instantiated with the FileDataSource we just created
            DataHandler fileData = new DataHandler(fds);

            //the BodyPart will contain the word processing file
            bpart2.setDataHandler(fileData);

            //add the second BodyPart, the one containing the attachment, to the Multipart object
            multipart.addBodyPart(bpart2);

            //finally, set the content of the MimeMessage to the Multipart object
