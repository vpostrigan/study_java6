...
/**
 * <p>Это форма для изменения продуктов</p>
 * @author Postrigan V.A.
 * @version 1.0 
 */
public class EditProductForm extends ActionForm {
	...
	public ActionErrors validate(ActionMapping mapping, HttpServletRequest request) {
		...
		// Проверка поля 'code'
        if (code < 1){        
        	errors.add(finals.CardFileFinal.ERROR_CODE_EDIT, new ActionMessage(finals.CardFileFinal.ERROR_MODIFY_PRODUCT__CODE, code));
        }
        
        // Проверка поля 'productName'
        if (productName == null || "".equals(productName)){
        	errors.add(finals.CardFileFinal.ERROR_PRODUCT_NAME_EDIT, new ActionMessage(finals.CardFileFinal.ERROR_MODIFY_PRODUCT__PRODUCT_NAME, productName));
        }
        ...
        try{
        	...
        	if (priceValue < 0){
            	...
            	errors.add(finals.CardFileFinal.ERROR_PRICE_EDIT, new ActionMessage(finals.CardFileFinal.ERROR_MODIFY_PRODUCT__PRICE_NUMBER, price));
            }
        }catch(Exception e){
        	// Вместо цены введена строка
        	errors.add(finals.CardFileFinal.ERROR_PRICE_EDIT, new ActionMessage(finals.CardFileFinal.ERROR_MODIFY_PRODUCT__PRICE_STRING, price));
        }
